...
async def init_db(db_config: dict, size: int):

    try:
        pool = await asyncpg.create_pool(dsn=db_config, max_size=size)
    ...
    try:
        ...
        log.exception(f"Loading schema failed.", exc_info=error)
